...
def naiveSoftmaxLossAndGradient(
    ...
    y_probd = softmax(np.dot(outsideVectors, centerWordVec))
    
    y = np.zeros_like(y_probd)
    y[outsideWordIdx] = 1
    
    loss = -np.log(y_probd[outsideWordIdx])
    
    gradCenterVec = np.dot(y_probd - y, outsideVectors)
    gradOutsideVecs = np.outer(y_probd - y, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    y_probd = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    loss = -np.log(y_probd)
    
    gradCenterVec = -np.dot(1 -y_probd, outsideVectors[outsideWordIdx])
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = -np.dot(1 -y_probd, centerWordVec)

    for i in range(1,K+1):
        ws = indices[i]
        y_probd_ws = sigmoid(-np.dot(outsideVectors[ws], centerWordVec))
        loss += -np.log(y_probd_ws)
        gradOutsideVecs[ws] += np.dot(1.0 - y_probd_ws, centerWordVec)
        gradCenterVec += np.dot(1.0 - y_probd_ws, outsideVectors[ws])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentCenterWord_index = word2Ind[currentCenterWord]
    centerWordVector = centerWordVectors[currentCenterWord_index]

    for outsideWord in outsideWords:
        outsideWord_index = word2Ind[outsideWord]
        loss_now, gradCenterVec, gradOutsideVector =word2vecLossAndGradient(centerWordVector,outsideWord_index,           outsideVectors,dataset)
        loss += loss_now
        gradCenterVecs[currentCenterWord_index] += gradCenterVec
